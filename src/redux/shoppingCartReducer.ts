...
const updateBookItem = (selectedBooks: ShoppingCartBookItemType[], item: ShoppingCartBookItemType, existingBookIndex: number): ShoppingCartBookItemType => {
...
const updateSelectedBooks = (selectedBooks: ShoppingCartBookItemType[], newBook: ShoppingCartBookItemType, existingBookIndex: number) => {
...
const updateOrder = (state: ShoppingCartStateType, action: ShoppingCartActionsType) => {
    const existingBookIndex = state.selectedBooks.findIndex(({ book }) => book.isbn13 === action.payload.book.isbn13);
